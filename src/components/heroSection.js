...
function HeroSection() {
    ...
    return (
        <header className="w-full h-3/4 flex justify-center mt-6">
            <div id="carousel-content" className="border-double border-4  border-DarkBrown relative"  >
                <div id="carousels" >
                {image.map((ele)=>{
                        return <div style={{width:"1897px" ,height: "100%" }}  key={image.indexOf(ele)} > <Image src={ele} key={image.indexOf(ele)}  width={1897} height={720}  alt=""/> </div>
                })}
                </div>
            </div>
            <p className="text-white w-1/2 text-7xl absolute top-3/4 left-60 font-light" >Your <span className="text-brown">Stylish Furniture</span>  Marketplace</p>
        </header>
    );
}
